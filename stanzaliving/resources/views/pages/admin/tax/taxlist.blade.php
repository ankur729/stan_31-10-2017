     ...
     <div class="row">
                 ...
                 <div class="col-xs-3">
                  <div class="form-group">
                    <label> &nbsp;</label><Br>
                     ...
                     <a href=" {{ url('admin/taxes/add') }}" class="btn btn-success pull-left" style="margin:15px;display: block;float:right; ">Add New Tax</a>
                   </div>
                 </div>
               </div>
             </div>
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          @foreach($taxes as $key=>$ta)
                  <tr>
                    <td>{{$key+1}}</td>
                    <td>{{$ta->name}}</td>
                    <td>{{$ta->percent}}</td>
                    <td>
          <a title="Edit" href="{{action('TaxController@edit',['id'=>$ta->id])}}"><i class="fa fa-edit" aria-hidden="true"></i></a>
        <!--  <a data-toggle="modal" data-target="#myModal"><i class="fa fa-trash md-trigger" ></i></a>

@{{action('AccessController@edit',[$role->id])}}
         -->
     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
           <a title="Delete" href="{{action('TaxController@destroy',['id'=>$ta->id])}}" data-method="delete" ><i class="fa fa-trash-o" aria-hidden="true"></i></a>
           </td>
                  </tr>
             @endforeach
                        </tbody>
